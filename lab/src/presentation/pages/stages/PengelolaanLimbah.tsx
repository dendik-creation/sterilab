...
import { PROCEDURE_STEPS } from '../../../data/stages/pengelolaanLimbah';
import type { ProcedureStep } from '../../../data/stages/pengelolaanLimbah';
...
import {
  ...
  srOnly,
  textBase,
} from './pengelolaan-limbah/geometry';
import { ProcedureCard } from './pengelolaan-limbah/ProcedureCard';
import { SuccessNote } from './pengelolaan-limbah/SuccessNote';
import { PROCEDURES } from './pengelolaan-limbah/steps';
import type { ProcedureFrame, ProcedureRuntime } from './pengelolaan-limbah/types';

// Stage 3 - Pengelolaan Limbah Laboratorium. Figma "Sterilab-APHP" frame
// "6.1 - A/B" (node-id 305-616), 1920x1080. Only Langkah 1 is authored so far
// (see data/stages/pengelolaanLimbah.ts); the shell already walks
// PROCEDURE_STEPS generically, so the remaining three arrive as data + a
// workspace component each, exactly like Stage 2 and Stage 4 before them.
//
// This file is the *shell*, and only the shell - same split as
// KulturMikroba.tsx and TeknikAseptikPage.tsx: the top bar, the PROSEDUR
// tracking card, the success note, the live region and the walk through
// PROCEDURE_STEPS. The workspace in the middle is a component of its own
// under pengelolaan-limbah/steps/, picked by the step's id.
...
// The level Evaluasi unlocks once this Stage's last authored step finishes
// (core/progress/levelProgress.ts: 1 = Teknik Kerja Aseptik, 2 = Pembuatan
// Media Kultur Mikroba, 3 = Pengelolaan Limbah Laboratorium).
...
export function PengelolaanLimbah() {
  ...
  return (
    ...
    >
      ...
      <ProcedureCard
        ...
        showDots={!isMobile}
        animation={anim(1)}
        // Langkah 4's frame (Figma group 313:1960) moves the card to the
        // right of the bench to leave room for its three sorting bins on
        // the left; every other Langkah keeps the component's own default.
        position={step.kind === 'sort' ? { x: 1356.096, y: 471.96 } : undefined}
      />
